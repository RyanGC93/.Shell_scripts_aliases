...
# git@github.com:gko/vimio.git
	# Creates github repo    
		...
		clear 
		echo "Repo Made @ git@github.com:${repoName}"
	    echo "make local directory with git directory, .gitignore, and README?"
	    echo "if <NO> there an option to change remote to created repo"
		echo
			options=("yes" "no" )
			select_option "${options[@]}"
			choice=$?
